    ...
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
      <div>
        ...
        <input required value={form.company_name} onChange={set('company_name')}
          className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent"
      ...
      <div>
        ...
        <input required value={form.entity_type} onChange={set('entity_type')}
          className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent"
          placeholder="e.g. Pvt Ltd, LLC, Partnership" />
      </div>
...
export default function Clients() {
  ...
  const handleCreate = async (form) => {
    setSaving(true);
    try { await createClient(form); setShowCreate(false); load(); }
    finally { setSaving(false); }
  };

  const handleUpdate = async (form) => {
    setSaving(true);
    try { await updateClient(editing.id, form); setEditing(null); load(); }
    finally { setSaving(false); }
  };

  const handleDelete = async () => {
    setSaving(true);
    try { await deleteClient(deleting.id); setDeleting(null); load(); }
    finally { setSaving(false); }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">{clients.length} client{clients.length !== 1 ? 's' : ''} total</p>
        </div>
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)}
          ...
          className="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent bg-white" />
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(client => (
            <div key={client.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow p-5">
              ...
              <div className="mt-3">
                <h3 className="font-semibold text-slate-800 text-sm leading-tight">{client.company_name}</h3>
                <p className="text-xs text-slate-500 mt-1">{client.country}</p>
                <span className="inline-block mt-2 text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{client.entity_type}</span>
              </div>
              <Link to={`/clients/${client.id}`}
                className="mt-4 flex items-center gap-1 text-xs text-indigo-500 hover:text-indigo-700 font-medium transition-colors">
